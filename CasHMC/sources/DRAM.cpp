...
namespace CasHMC
{
  ...
  DRAM::DRAM(ofstream &debugOut_, ofstream &stateOut_, unsigned id, VaultController *contP, string headerPrefix):
    SimulatorObject(debugOut_, stateOut_),
    DRAMID(id),
    vaultContP(contP)
  {
    classID << DRAMID;
    header = "        (" + headerPrefix + "_DR_" + classID.str() + ")";
  ...
  void DRAM::receiveCMD(DRAMCommand *recvCMD)
  {
    switch(recvCMD->commandType) {
      case ACTIVATE:
        ...
        for(int b=0; b<NUM_BANKS; b++) { //FAST: Jiayi, why not rewrite it to bankStates[recvCMD->bank]? no loop, faster
          if(recvCMD->bank != b) {
      ...
      case READ_P:
        ...
        if(recvCMD->commandType == READ_P) { //FAST: Jiayi, no need for IF
