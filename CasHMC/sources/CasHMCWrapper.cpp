...
namespace CasHMC
{
  ...
  //
  bool CasHMCWrapper::ReceiveTran(TransactionType tranType, uint64_t addr, unsigned size, int cpu_id = 0)
  {
  ...
  bool CasHMCWrapper::ReceiveTran(Transaction *tran, int cpu_id = 0)
  {
    cout << "HMC Controllers queue size: " << hmcConts.size() << endl;
    if (hmcConts[cpu_id] == NULL) {
      cout << "NO HMC Controller" << endl;
      assert(0);
    }
    if(hmcConts[cpu_id]->ReceiveDown(tran)) {
      //if (tran->transactionType == ACTIVE_ADD) {  // Jiayi, 02/07
      //    outstandRequests.push_back(tran->src_address);
      //}
