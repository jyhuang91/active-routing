...
//      envp[1] = (char *)(string("LD_LIBRARY_PATH=")+string(ld_library_path)).c_str();
      ...
      char ** argp = new char * [programs[i].prog_n_argv.size() + 20];
      int  curr_argc = 0;
      ...
      argp[curr_argc++] = (char *) "-injection";
      argp[curr_argc++] = (char *) "child";
  ...
  delete [] num_fetched_instrs;

  if (old_mapping) delete [] old_mapping;
  if (old_mapping_inv) delete [] old_mapping_inv;
  if (new_mapping) delete [] new_mapping;

  for (uint32_t i = 0; i < programs.size(); i++) {
    delete [] programs[i].buffer;
  }
  delete pts;
  /*recvfrom(programs[0].sockfd, programs[0].buffer, sizeof(PTSMessage), 0, (struct sockaddr *)&(programs[0].my_addr), &addr_len);
