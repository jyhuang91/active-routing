...
extern void bpnn(float *il, float *hl, float *ol, float **iw, float **hw, int in, int hid, int out, float *delta_o, float *target, float *delta_h, float **old_hw, float **old_iw);
...
void bpnn_train_kernel(BPNN *net, float *eo, float *eh)
{
  ...
  /*** put into one function - Start ***/
  bpnn(net->input_units, net->hidden_units, net->output_units, net->input_weights, net->hidden_weights,
      in, hid, out, net->output_delta, net->target, net->hidden_delta, net->hidden_prev_weights, net->input_prev_weights);
  /*** put into one function - End ***/
  /*
  ...
  printf("(From kernel) Output err: %f and hidden err: %f\n", out_err, hid_err);
  */
