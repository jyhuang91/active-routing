...
int n;
{
  ...
  //if (new == NULL)
  if (posix_memalign((void **) &new, PAGE_SIZE, n * sizeof(float)))
  {
...
int m, n;
{
  ...
  //if (new == NULL)
  if (posix_memalign((void **) &new, PAGE_SIZE, m * sizeof(float *)))
  {
...
void *bpnn_pthread_worker(void *args)
{
  ...
  for (j = start_hid; j < end_hid; j++) {
    ...
    if (0 < rr_start) {
      ...
      if (lines == 0) lines = 1;
      UpdateRRPage(&iw[j][k], &il[k], &hl[j], lines, FMULT);
    }
    lines = PAGE_SIZE / CACHELINE_SIZE;
    for (k = rr_start; k < rr_end; k += stride) {
      UpdateRRPage(&iw[j][k], &il[k], &hl[j], lines, FMULT);
    ...
    if (rr_end < (in + 1)) {
      lines = ((in + 1) - rr_end) / CACHELINE_SIZE;
      if (lines == 0) lines = 1;
      UpdateRRPage(&iw[j][k], &il[k], &hl[j], lines, FMULT);
    }
    Gather(0, 0, &hl[j], 1);
  ...
  for (j = start_out; j < end_out; j++) {
    ...
    if (0 < rr_start) {
      lines = (rr_start - 0) / CACHELINE_SIZE;
      if (lines == 0) lines = 1;
      UpdateRRPage(&hw[j][k], &hl[k], &ol[j], lines, FMULT);
    }
    lines = PAGE_SIZE / CACHELINE_SIZE;
    for (k = rr_start; k < rr_end; k += stride) {
      UpdateRRPage(&hw[j][k], &hl[k], &ol[j], lines, FMULT);
    ...
    if (rr_end < (hid + 1)) {
      lines = ((hid + 1) - rr_end) / CACHELINE_SIZE;
      if (lines == 0) lines = 1;
      UpdateRRPage(&hw[j][k], &hl[k], &ol[j], lines, FMULT);
    }
    Gather(0, 0, &ol[j], 1);
