...
class MD:
    ...
    def __init__(self, mdfile, ticks_per_cycle):
        ...
        if mdfile == "/dev/null":
            ...
            self.ticks_per_cycle = int(
                self.mdparams['pts.l1i$.process_interval'])
            print("md_file: " + str(self.ticks_per_cycle))
            self.num_hthreads = int(self.mdparams['pts.num_hthreads'])
            print("md_file: num hthreads: " + str(self.num_hthreads))
            self.max_ipc = int(self.mdparams['pts.num_hthreads']) / int(
                self.mdparams['pts.num_hthreads_per_l1$'])
            self.num_ranks_per_MC = int(
                self.mdparams['pts.mc.num_ranks_per_mc'])
...
class Stat:
    def __init__(self, md):
        ...
        self.num_hthreads = md.num_hthreads
    ...
    def parse(self, logfilename, ipcfile):
        # parse logfile
        self.processing_dram = 0
        try:
        ...
        if logfilename.find("dram_log") != -1:
            self.processing_dram = 1

        for line in logfile:
            if re.search("^  -- ", line) != None and re.search(
                    "^  -- \[", line) == None:
                line = line[5:]
                #print("line is:"+ str(line))
                temp = re.split('\s*', line)
                #print("temp is :" + str(temp))
                if temp[1][:1] == '[' and temp[1][-1:] == ']':
                    temp.insert(1, '[')
                    temp[2] = temp[2][1:]

                if line[0:4] == 'L1$I' and temp[0][-1:] == ']':
                    temp.insert(1, 'dummy')
                elif line[0:4] == 'L1$D' and temp[0][-1:] == ']':
                    temp.insert(1, 'dummy')

                if line[0:4] == 'MC  ' and temp[9][:1] == '(' and temp[30][:1] == '(':
                    temp.insert(9, '(')
                    temp[10] = temp[10][1:]
                    temp.insert(31, '(')
                    temp[32] = temp[32][1:]
                ...
                if line[0:3] == 'HTH':
                    #print 'Processing HTH ...'
                    #print("logfile is:" + logfilename)
                    #print("line is:"+ str(line))
                    #print("temp is :" + str(temp))
                    self.num_hthreads = self.num_hthreads + 1
                    self.num_br_misses = self.num_br_misses + int(
                        temp[7][1:][:-1])
                    ...
                    if (len(temp) >= 28):
                        self.total_mem_wr_time = self.total_mem_wr_time + int(
                            temp[25][:-1])
                        self.total_mem_rd_time = self.total_mem_rd_time + int(
                            temp[28])
                elif line[0:3] == 'NoC':
                    #print 'Processing NoC ...'
                    #print("logfile is:" + logfilename)
                    #print("line is:"+ str(line))
                    #print("temp is :" + str(temp))
                    if temp[10][-2] == ')':
                        self.xbar_accs = int(temp[8][1:][:-1]) + int(
                            temp[9][:-1]) + int(temp[10][:-2])
                    else:
                        self.xbar_accs = int(temp[7][1:][:-1]) + int(
                            temp[8][:-1]) + int(temp[9][:-2])
                        #print("reading L1 I ||| +++...")
                elif line[0:4] == 'L1$I':
                    ...
                    if temp[3] == 'RD':
                        #print("reading L1 I ...")
                        self.num_L1I_accs = self.num_L1I_accs + int(
                            temp[7][:-1].replace(")", ""))
                        self.num_L1I_misses = self.num_L1I_misses + int(
                            temp[6][0:][:-1])
                    elif temp[3] == '(ev_coherency,':
                        #print("reading coherency event ...")
                        self.num_L1I_ev_coh = self.num_L1I_ev_coh + int(
                            temp[6][0:][:-1])
                        self.num_L1I_coh_accs = self.num_L1I_coh_accs + int(
                            temp[7][:-1])
                elif line[0:4] == 'L1$D':
                    #print 'Processing L1$D ...'
                    #print("logfile is:" + logfilename)
                    #print("line is:"+ str(line))
                    #print("temp is :" + str(temp))
                    #print("reading L1D ..............")
                    if temp[3] == 'RD':
                        #print("L1D read ++++++++++")
                        self.num_L1D_rd_misses = self.num_L1D_rd_misses + int(
                            temp[6][:-1])
                        self.num_L1D_rd_accs = self.num_L1D_rd_accs + int(
                            temp[7][:-1].replace(")", ""))
                    elif temp[3] == 'WR':
                        #print("L1D write ||||||| ++++++++++")
                        self.num_L1D_wr_misses = self.num_L1D_wr_misses + int(
                            temp[6][:-1])
                        self.num_L1D_wr_accs = self.num_L1D_wr_accs + int(
                            temp[7][:-1].replace(")", ""))
                    elif temp[3] == '(ev_coherency,':
                        self.num_L1D_ev_coh = self.num_L1D_ev_coh + int(
                            temp[9][0:][:-1])
                        self.num_L1D_ev_cap = self.num_L1D_ev_cap + int(
                            temp[10][:-1])
                        self.num_L1D_coh_accs = self.num_L1D_coh_accs + int(
                            temp[11][:-1])
                        self.num_L1D_nacks = self.num_L1D_nacks + int(
                            temp[14][:-2])
                elif line[0:4] == 'TLBI':
                    #print 'Processing TLBI ...'
                    #print("logfile is:" + logfilename)
                    #print("line is:"+ str(line))
                    #print("temp is :" + str(temp))
                    self.num_TLBI_misses = self.num_TLBI_misses + int(
                        temp[5][1:][:-1])
                    self.num_TLBI_accs = self.num_TLBI_accs + int(temp[6][:-1])
                elif line[0:4] == 'TLBD':
                    #print 'Processing TLBD ...'
                    #print("logfile is:" + logfilename)
                    #print("line is:"+ str(line))
                    #print("temp is :" + str(temp))
                    self.num_TLBD_misses = self.num_TLBD_misses + int(
                        temp[5][1:][:-1])
                    ...
                    #print("line is:"+ str(line))
                    #print("temp is :" + str(temp))
                    #print("reading L2 cache ...")
                    if temp[4] == 'RD':
                        #print("L2 Read ...")
                        self.num_L2_rd_misses = self.num_L2_rd_misses + int(
                            temp[7][0:][:-1])
                        self.num_L2_rd_accs = self.num_L2_rd_accs + int(
                            temp[8][:-1].replace(")", ""))
                    elif temp[4] == 'WR':
                        #print("L2 Write ...")
                        self.num_L2_wr_misses = self.num_L2_wr_misses + int(
                            temp[7][0:][:-1])
                        self.num_L2_wr_accs = self.num_L2_wr_accs + int(
                            temp[8][:-1].replace(")", ""))
                    elif temp[4][:3] == '(ev':
                        #print("reading coherency stats l2 ...")
                        self.num_L2_ev_coh = self.num_L2_ev_coh + int(
                            temp[10][0:][:-1])
                        self.num_L2_ev_cap = self.num_L2_ev_cap + int(
                            temp[11][:-1])
                        self.num_L2_coh_accs = self.num_L2_coh_accs + int(
                            temp[12][:-1])
                elif line[0:4] == 'MC  ':
                    #print 'Processing MC ...'
                    #print("logfile is:" + logfilename)
                    ...
                    self.num_MC_rds = self.num_MC_rds + int(temp[10][:-1])
                    self.num_MC_wrs = self.num_MC_wrs + int(temp[11][:-1])
                    self.num_MC_acts = self.num_MC_acts + int(temp[12][:-1])
                    self.num_MC_pres = self.num_MC_pres + int(temp[13][:-2])
                    #self.num_RBoL_hits    = self.num_RBoL_hits   + int(temp[36][1:][:-1])
                    #self.num_RBoL_misses  = self.num_RBoL_misses + int(temp[37][:-1])
                    self.num_DRAM_pages = self.num_DRAM_pages + int(temp[23])
                    if len(temp) >= 20:
                        self.active_pd_time = self.active_pd_time + int(
                            temp[19][:-1])
                elif line[0:3] == 'Dir':
                    #print("reading directories 1111.....................................")
                    ...
                    if temp[4] == '(i->tr,':
                        ...
                        self.num_Dir_i_to_tr = self.num_Dir_i_to_tr + int(
                            temp[14][1:][:-1])
                        self.num_Dir_e_to_tr = self.num_Dir_e_to_tr + int(
                            temp[15][:-1])
                        self.num_Dir_s_to_tr = self.num_Dir_s_to_tr + int(
                            temp[16][:-1])
                        self.num_Dir_m_to_tr = self.num_Dir_m_to_tr + int(
                            temp[17][:-1])
                        self.num_Dir_m_to_i = self.num_Dir_m_to_i + int(
                            temp[18][:-1])
                        self.num_Dir_tr_to_i = self.num_Dir_tr_to_i + int(
                            temp[19][:-1])
                        self.num_Dir_tr_to_e = self.num_Dir_tr_to_e + int(
                            temp[20][:-1])
                        self.num_Dir_tr_to_s = self.num_Dir_tr_to_s + int(
                            temp[21][:-1])
                        self.num_Dir_tr_to_m = self.num_Dir_tr_to_m + int(
                            temp[22][:-1])
                    else:
                        if self.processing_dram == 1:
                            self.num_Dir_nacks = self.num_Dir_nacks + int(
                                temp[14][1:][:-1])
                            self.num_Dir_bypass = self.num_Dir_bypass + int(
                                temp[15][:-1])
                            self.num_Dir_evict = self.num_Dir_evict + int(
                                temp[16][:-1])
                            self.num_Dir_invalidate = self.num_Dir_invalidate + int(
                                temp[17][:-1])
                            self.num_Dir_from_mc = self.num_Dir_from_mc + int(
                                temp[18][:-1])
                            self.num_Dir_cache_acc = self.num_Dir_cache_acc + int(
                                temp[19][:-1])
                            self.num_Dir_cache_miss = self.num_Dir_cache_miss + int(
                                temp[20][:-1])
                        else: 
                            self.num_Dir_nacks = self.num_Dir_nacks + int(
                                temp[16][1:][:-1])
                ...
                elif re.search("total number of ticks: ", line) != None:
                    #print 'Processing total number of ticks ...'
                    #print("logfile is:" + logfilename)
                    #print("line is:"+ str(line))
                    #print("temp is :" + str(temp))
                    self.num_ticks = int(temp[4])
                    #print("number_of_ticks ..... " + str(self.num_ticks))
                elif re.search("total number of fetched instructions",
                               line) != None:
                    #print 'Processing total number of fetched instructions ...'
                    #print("logfile is:" + logfilename)
                    #print("line is:"+ str(line))
                    #print("temp is :" + str(temp))
                    ...
                    #print("line is:"+ str(line))
                    #print("temp is :" + str(temp))
                    self.num_cond_broadcast = self.num_cond_broadcast + int(
                        temp[7][1:][:-1])
                    self.num_cond_signal = self.num_cond_signal + int(
                        temp[8][:-1])
                ...
                elif re.search("total number of mem acc", line) != None:
                    #print 'Processing total number of mem acc ...'
                    #print("logfile is:" + logfilename)
                    #print("line is:"+ str(line))
                    #print("temp is :" + str(temp))
                    #print("total number of memory access: " + str(temp[6]))
                    self.num_mem_accs = self.num_mem_accs + int(temp[6])
                elif re.search("total number of updates :", line) != None:
                    #print 'Processing total number of updates ...'
                    #print("logfile is:" + logfilename)
                    #print("line is:"+ str(line))
                    #print("temp is :" + str(temp))
                    self.num_updates = self.num_updates + int(temp[5])
                elif re.search("total number of gathers :", line) != None:
                    #print 'Processing total number of gathers ...'
                    #print("logfile is:" + logfilename)
                    #print("line is:"+ str(line))
                    #print("temp is :" + str(temp))
                    self.num_gathers = self.num_gathers + int(temp[5])

            elif ipcfile != '/dev/null' and re.search(
                    "\sinstructions are fetched so", line) != None:
                ...
                temp = re.split('\s*', line[6:].lstrip())
                self.num_instrs_spin = self.num_instrs_spin + int(
                    temp[21][:-1])
        ...
        if ipcfile != '/dev/null':
            ...
            for curr_cycle in self.cycles:
                if i % 10 == 0 or subsampling == False:
                    ...
                    curr_ipc = 1.0 * (curr_instr - prev_instr) / (
                        curr_cycle - prev_cycle)
                    ipcfile.write(
                        str(curr_instr / 10.0**9) + ' ' + str(curr_ipc) + ' ' +
                        str(1.0 * curr_instr / curr_cycle) + ' ' +
                        str(curr_cycle / 10.0**6) + '\n')
    ...
    def compute_energydelay(self, chipkill=False):
        ...
        if chipkill == False:
            ...
            if self.vmd_page_sz == '65536':
                ...
                bus_act = 1.5 * 1.5 * (7 + 18) * 1.3 * (102 * 0.1 + 2 *
                                                        (4 + 16)) / 1000  # nJ
                bus_rd = bus_act + 42.51 * 512 / 2000  # nJ
                bus_wr = bus_act + 72.80 * 512 / 2000  # nJ
                ...
                main_act_dyn = (
                    0.099 + 1.067) * 11 * 2  #(0.112 + 1.067)*11*2  # nJ
                ...
                bus_act = 1.5 * 1.5 * (7 + 19) * 1.3 * (102 * 0.1 + 2 *
                                                        (4 + 7)) / 1000  # nJ
                bus_rd = bus_act + 42.51 * 512 / 2000  # nJ
                bus_wr = bus_act + 72.80 * 512 / 2000  # nJ
        ...
        l2cache_power = 0
        l2cache_power = l2cache_power + (
            self.num_L1I_accs + self.num_L1I_ev_coh) * (
                itlb + l1i_rd_dyn + l1i_rd_dyn_tag) / (clk * curr_cycle)
        l2cache_power = l2cache_power + (
            self.num_L1I_coh_accs - self.num_L1I_ev_coh) * l1i_rd_dyn_tag / (
                clk * curr_cycle)
        l2cache_power = l2cache_power + (
            self.num_L1D_rd_accs + self.num_L1D_ev_coh + self.num_L1D_ev_cap
        ) * (dtlb + l1d_rd_dyn + l1d_rd_dyn_tag) / (clk * curr_cycle)
        l2cache_power = l2cache_power + (
            self.num_L1D_ev_coh + self.num_L1D_ev_cap) * dwbb / (
                clk * curr_cycle)
        l2cache_power           = l2cache_power + (self.num_L1D_coh_accs + self.num_L1D_ev_cap + \
                                           self.num_L1D_wr_misses + self.num_L1I_coh_accs + \
                                           self.num_L1I_misses) * intra_dir_dyn / (clk * curr_cycle)
        l2cache_power += (self.num_L2_rd_accs + self.num_L1D_wr_accs) * (
            l1d_wr_dyn + l1d_wr_dyn_tag) / (clk * curr_cycle)
        l2cache_power = l2cache_power + (
            self.num_L1D_coh_accs - self.num_L1D_ev_coh - self.num_L1D_ev_cap
        ) * l1d_rd_dyn_tag / (clk * curr_cycle)
        l2cache_power = l2cache_power + (
            self.num_L2_rd_accs + self.num_L2_ev_coh + self.num_L2_ev_cap) * (
                missb + fillb + prefetchb + l2_rd_dyn + l2_rd_dyn_tag) / (
                    clk * curr_cycle)
        l2cache_power = l2cache_power + (
            self.num_L2_coh_accs - self.num_L2_ev_coh - self.num_L2_ev_cap
        ) * l2_rd_dyn_tag / (clk * curr_cycle)
        l2cache_power = l2cache_power + (
            self.num_MC_rds + self.num_L2_wr_accs) * (
                l2_wr_dyn + l2_wr_dyn_tag) / (clk * curr_cycle)
        l2cache_power           = l2cache_power + (L2_mfbuffer_dyn*2*self.num_L2_wr_misses +\
        ...
        #    self.num_L2_coh_accs - self.num_L2_ev_coh - self.num_L2_ev_cap
        #) * l2_rd_dyn_tag / (clk * curr_cycle)
        #l2cache_power += (self.num_MC_rds + self.num_L2_wr_accs) * (
        #    l2_wr_dyn + l2_wr_dyn_tag) / (clk * curr_cycle)
        ...
        cpu_power = cpu_power + (self.num_L1I_accs + self.num_L1I_ev_coh) * (
            itlb + l1i_rd_dyn + l1i_rd_dyn_tag) / (clk * curr_cycle)
        cpu_power = cpu_power + (self.num_L1I_coh_accs - self.num_L1I_ev_coh
                                 ) * l1i_rd_dyn_tag / (clk * curr_cycle)
        cpu_power = cpu_power + (
            self.num_L1D_rd_accs + self.num_L1D_ev_coh + self.num_L1D_ev_cap
        ) * (dtlb + l1d_rd_dyn + l1d_rd_dyn_tag) / (clk * curr_cycle)
        cpu_power = cpu_power + (self.num_L1D_ev_coh + self.num_L1D_ev_cap
                                 ) * dwbb / (clk * curr_cycle)
        cpu_power           = cpu_power + (self.num_L1D_coh_accs + self.num_L1D_ev_cap + \
                                           self.num_L1D_wr_misses + self.num_L1I_coh_accs + \
                                           self.num_L1I_misses) * intra_dir_dyn / (clk * curr_cycle)
        cpu_power = cpu_power + (
            self.num_L2_rd_accs + self.num_L1D_wr_accs) * (
                l1d_wr_dyn + l1d_wr_dyn_tag) / (clk * curr_cycle)
        cpu_power = cpu_power + (
            self.num_L1D_coh_accs - self.num_L1D_ev_coh - self.num_L1D_ev_cap
        ) * l1d_rd_dyn_tag / (clk * curr_cycle)
        ...
        cpu_power = cpu_power + (self.num_MC_rds + self.num_L2_wr_accs) * (
            l2_wr_dyn + l2_wr_dyn_tag) / (clk * curr_cycle)
        ...
        cpu_power = cpu_power + Local_xbar_dyn * traffic_through_intra_crossbar / (
            clk * curr_cycle)
        ...
        cpu_power = cpu_power + self.num_Dir_accs * dir_dyn / (
            clk * curr_cycle)
        cpu_power = cpu_power + (self.xbar_accs) * l2_main_xbar_wr_dyn / (
            clk * curr_cycle)
        ...
        mem_standby_power = main_standby * self.num_MCs * self.num_ranks_per_MC * (
            (1 - pd_perc) + pd_perc * pd_power_scale)
        ...
        mem_dynamic_power = (
            self.num_MC_acts * main_act_dyn / 2 + self.num_MC_rds * main_rd_dyn
            + self.num_MC_wrs * main_wr_dyn) / (clk * curr_cycle)
        mem_dynamic_power = (
            self.num_MC_acts *
            (main_act_dyn + MC_logic_dyn) / 2 + self.num_MC_rds *
            (main_rd_dyn + MC_logic_dyn) + self.num_MC_wrs *
            (main_wr_dyn + MC_logic_dyn)) / (clk * curr_cycle)

        mem_io_power = (
            self.num_MC_acts * bus_act + self.num_MC_rds * bus_rd +
            self.num_MC_wrs * bus_wr) / (
                clk * curr_cycle) / 2  # todo -- /2 is a hack currently
        energy_delay = (
            cpu_power + mem_standby_power + mem_refresh_power +
            mem_dynamic_power + mem_io_power) * curr_cycle * curr_cycle

        return (cpu_power, mem_standby_power, mem_refresh_power,
                mem_dynamic_power, mem_io_power, energy_delay, pd_perc,
                self.num_mem_accs, self.num_updates, self.num_gathers,
                curr_cycle, l2cache_power)
        ...
        print self.md.mdparams['pts.mc.scheduling_policy'], self.md.mdparams[
            'pts.mc.vmd_page_sz'], self.md.mdparams[
                'pts.mc.num_cached_pages_per_bank'], self.num_instrs, self.num_MC_rds + self.num_MC_wrs, self.num_MC_acts, fpformat.fix(
                    1.0 * self.num_instrs *
                    self.ticks_per_cycle / self.num_ticks, 2), fpformat.fix(
    ...
    def show(self):
        ...
        print ' - miss rate = ' + fpformat.fix(
            100.0 * self.num_L1I_misses / self.num_L1I_accs, 2) + '%'

        print 'L1D'
        print ' - # of RD requests = ' + str(self.num_L1D_rd_accs)
        print ' - # of WR requests = ' + str(self.num_L1D_wr_accs)
        print ' - # of coherency (evictions, accesses) = (' + str(
            self.num_L1D_ev_coh) + ', ' + str(self.num_L1D_coh_accs) + ')'
        print ' - # of capacity evictions = ' + str(self.num_L1D_ev_cap)
        print ' - RD miss rate = ' + fpformat.fix(
            100.0 * self.num_L1D_rd_misses / self.num_L1D_rd_accs, 2) + '%'
        print ' - WR miss rate = ' + fpformat.fix(
            100.0 * self.num_L1D_wr_misses / self.num_L1D_wr_accs, 2) + '%'
        print ' - coherency access ratio = ' + fpformat.fix(
            100.0 * self.num_L1D_coh_accs /
            (self.num_L1D_coh_accs + self.num_L1D_rd_accs +
             self.num_L1D_wr_accs), 2) + '%'

        print 'L2'
        print ' - # of RD requests = ' + str(self.num_L2_rd_accs)
        print ' - # of WR requests = ' + str(self.num_L2_wr_accs)
        print ' - # of coherency (evictions, accesses) = (' + str(
            self.num_L2_ev_coh) + ', ' + str(self.num_L2_coh_accs) + ')'
        print ' - # of capacity evictions = ' + str(self.num_L2_ev_cap)
        print ' - RD miss rate = ' + fpformat.fix(
            100.0 * self.num_L2_rd_misses / self.num_L2_rd_accs, 2) + '%'
        print ' - WR miss rate = ' + fpformat.fix(
            100.0 * self.num_L2_wr_misses / self.num_L2_wr_accs, 2) + '%'
        print ' - coherency access ratio = ' + fpformat.fix(
            100.0 * self.num_L2_coh_accs /
            (self.num_L2_coh_accs + self.num_L2_rd_accs + self.num_L2_wr_accs),
            2) + '%'
        ...
        if self.num_Dir_accs > 0:
            print 'Dir'
            print ' - # of accesses = ' + str(
                self.num_Dir_accs) + ', # of directory cache accesses = ' + str(
                    self.num_Dir_cache_acc)
            ...
            print ' - directory cache miss rate = ' + fpformat.fix(
                100.0 * self.num_Dir_cache_miss /
                (1 + self.num_Dir_cache_acc), 3) + ' %'

        if self.num_MC_rds > 0:
            print 'MC'
            print ' - # of RD requests = ' + str(self.num_MC_rds)
            print ' - # of WR requests = ' + str(self.num_MC_wrs)
            print ' - ACT / (RD + WR)  = ' + fpformat.fix(
                100.0 * self.num_MC_acts /
                (self.num_MC_rds + self.num_MC_wrs), 2) + '%'
        else:
            self.num_MC_rds = 1
            self.num_MCs = 1
        ...
        print ''
        print '# of instrs = ' + str(self.num_instrs) + ' = ' + fpformat.fix(
            1.0 * self.num_instrs / 10**9,
            3) + 'G,  # of x87 instrs = ' + fpformat.fix(
                1.0 * self.num_x87_ops / 10**9,
                3) + 'G,  # of instrs with spin loops = ' + fpformat.fix(
                    1.0 * self.num_instrs_spin / 10**9, 3) + 'G'
        print 'IPC         = ' + fpformat.fix(
            1.0 * self.num_instrs * self.ticks_per_cycle / self.num_ticks, 2)
        print 'Xbar BW     = ' + fpformat.fix(
            1.0 * self.xbar_accs * self.ticks_per_cycle / self.num_ticks,
            2) + ' accs/cycle'
        print 'MC  BW      = ' + fpformat.fix(
            1.0 * (self.num_MC_rds + self.num_MC_wrs) * self.ticks_per_cycle /
            self.num_ticks,
            3) + ' accs/cycle,  active PD time = ' + fpformat.fix(
                100.0 * self.active_pd_time /
                (self.num_MCs * self.num_ticks), 2) + ' %'
        print 'avg mem WR latency          = ' + fpformat.fix(
            1.0 * self.total_mem_wr_time / self.num_L1D_wr_accs /
            self.ticks_per_cycle, 3) + ' cycles'
        print 'avg mem RD latency          = ' + fpformat.fix(
            1.0 * self.total_mem_rd_time / self.num_L1D_rd_accs /
            self.ticks_per_cycle, 3) + ' cycles'
        print '# of mem ops per instr       = ' + fpformat.fix(
            1.0 *
            (self.num_L1D_rd_accs + self.num_L1D_wr_accs) / self.num_instrs, 3)
        print '# of branch misses per instr = ' + fpformat.fix(
            1.0 * self.num_br_misses / self.num_instrs, 5)
        print '# of TLBD misses per instr   = ' + fpformat.fix(
            1.0 * self.num_TLBD_misses / self.num_instrs, 5)
        print '# of TLBI misses per instr   = ' + fpformat.fix(
            1.0 * self.num_TLBI_misses / self.num_instrs, 5)

        print '(# of L2 misses) / (# of instructions) = ' + fpformat.fix(
            1.0 *
            (self.num_L2_rd_misses + self.num_L2_wr_misses) / self.num_instrs,
            4)
        print '(# MC accesses) / (# of instructions)  = ' + fpformat.fix(
            1.0 * (self.num_MC_rds + self.num_MC_wrs) / self.num_instrs, 4)
